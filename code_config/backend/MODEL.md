...
- **基类已包含的字段不允许在子类重复声明**：`BasicVersionModel` 已经提供 `id`、`created_at`、`updated_at`、`operator`、`operator_category`、`version`、`desc`；`BasicModel` 已提供前 5 个。需要"描述"含义直接复用 `desc`，不要再写 `desc: Optional[str] = Field(...)`
...
- 列表字段统一用 `_list` 后缀（如 `data_list`、`user_id_list`），禁止 `list`、`users`、`user_ids` 这类命名
