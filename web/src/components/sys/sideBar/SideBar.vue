    ...
    <div ref="sidebarRootRef" class="w-70 h-screen bg-white flex flex-col relative overflow-hidden sidebar-custom">
      ...
      <div class="pl-8 pb-3 pt-5 relative z-1">
        <div class="flex items-center gap-3.5">
          ...
          <span
              class="text-xl font-extrabold bg-gradient-to-br from-indigo-500 to-purple-600 bg-clip-text text-transparent tracking-tight logo-text">个人知识库平台</span>
        </div>
      </div>
      ...
      <div class="slot user-slot">
        <Transition name="slot-fade" mode="out-in">
          ...
          <div :key="isCollapsed ? 'mini' : 'full'" class="slot-content">
            <!-- 展开态：用于显示 + 测量高度 -->
            <div v-if="!isCollapsed" ref="userMeasureRef" class="px-6 pb-5 pt-3 relative z-1">
              <n-card class="bg-gradient-to-br from-gray-50 to-gray-100 border border-black/4 custom-card-content user-profile-card"
                      :bordered="false">
                <div class="user-profile-row">
                  ...
                  <div class="user-profile-text">
                    <h4 class="text-sm font-semibold text-gray-800 mb-0.5 overflow-hidden text-ellipsis whitespace-nowrap"
                        :title="userName">{{ userName }}</h4>
                    <p class="text-xs text-gray-500 font-medium overflow-hidden text-ellipsis whitespace-nowrap"
                       :title="userRoleLabel">{{ userRoleLabel }}</p>
                  </div>
                  <n-dropdown :options="userOptions" placement="top-end" :show-arrow="true"
                              @select="handleOptionSelect">
                    <n-button size="small" quaternary circle
                              class="w-8 h-8 text-gray-500 hover:bg-indigo-500 hover:text-white hover:-translate-y-0.5 hover:shadow-md hover:shadow-indigo-500/20 active:translate-y-0 transition-all ml-4"
            ...
            <!-- 折叠态微头像 -->
            <div v-else class="p-4 relative z-1 flex flex-col items-center gap-2">
...
const userOptions = ref(defaultUserOptions)
