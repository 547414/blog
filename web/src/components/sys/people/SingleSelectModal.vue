  ...
  >
    ...
    <div class="flex flex-col h-[700px]">
      ...
      <div class="px-6 bg-gray-50 border-b border-gray-100 selected-users-container">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-medium text-gray-700">已选择 {{ selectedUser?.name || '无' }}</span>
        ...
        <div class="selected-users-scroll">
          <div class="flex gap-2">
            <n-tag
                v-if="selectedUser"
                :key="selectedUser.unionUserUuid"
                type="primary"
                closable
                @close="handleClearSelected"
                size="small"
            >
              {{ selectedUser.name }}
            </n-tag>
    ...
    <template #footer>
      <div class="flex justify-between items-center">
        <span class="text-sm text-gray-500">单选人员</span>
...
const total = ref(0)
const selectedUser = ref<WebUserDetail | null>(null)
...
const columns: DataTableColumns<WebUserDetail> = [
  {
    ...
    align: 'center',
    render(row) {
      const isSelected = selectedUser.value?.unionUserUuid === row.unionUserUuid
      return h(NRadio, {
        checked: isSelected,
...
const getWebUserByIdList = (userIdList: string[]) => {
  ...
  apiGetWebUserByIdList(params).then((res) => {
    if (res.code === 200) {
      selectedUser.value = res.data?.[0] || null
    } else {
      notification.error({
        title: '获取用户列表失败',
        content: res.message,
...
const handleConfirm = () => {
  ...
  emit('selected', {selectType: selectType.value, selectedUser: selectedUser.value})
  close()
}
