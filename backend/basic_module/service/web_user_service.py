...
from basic_module.model.union_user_model import UnionUserModel, UnionUserLoginResultModel
...
from basic_module.model.web_user_model import WebUserLoginParamsModel, \
    ...
    UserRegisterParamsModel, ChangeCurrentUserRoleParamsModel, UserPageParamsModel, \
...
class WebUserService:
    ...
    @staticmethod
    def __get_role_data(role_data: Dict, user_id: str):
        if not role_data or not user_id:
            return None
        ...
        for key in key_list:
            if key in role_data:
                return role_data.get(key)

        normalized_user_id = user_id.replace('-', '').replace('_', '').lower()
        for key, value in role_data.items():
            if key.replace('-', '').replace('_', '').lower() == normalized_user_id:
                return value
    ...
    def user_login(
            self,
            params: WebUserLoginParamsModel,
            scene: str = "DEFAULT",
            space: str = "DEFAULT",
    ):
    ...
    def get_union_user_info(self, user_id: str, current_role_code: str = None, union_user_id: str = None):
        if union_user_id:
        ...
        for user in union_user.user_list:
            ...
            user_role_data = self.__get_role_data(
                role_data=role_data,
                user_id=user.union_user_user_id,
            )
            user.union_user_user_role_list = user_role_data.role_list if user_role_data else []
            if user_role_data and user_role_data.name:
                user.union_user_user_name = user_role_data.name
    ...
    def generate_user_token(
        ...
        self.__redis_client.set_value(
            ...
            refresh_token,
            expiration=int(timedelta(days=self.__config['jwt']['REFRESH_TOKEN_EXPIRE_DAYS']).total_seconds())
        )
    ...
    def refresh_user_token(
        ...
        if not refresh_token:
            raise BusinessError("登录状态已失效，请重新登录")

        # 将 Access Token 存入 Redis，Refresh Token 不在切换角色时轮换
        self.__redis_client.set_value(
            f"access_token_{space}:{jwt_payload.union_user_info.id}_{scene}",
            access_token,
            expiration=int(timedelta(minutes=self.__config['jwt']['ACCESS_TOKEN_EXPIRE_MINUTES']).total_seconds())
    ...
    def change_current_user_role(
        ...
        jwt_payload = JwtPayloadInfo(
            union_user_info=self.get_union_user_info(
                user_id=user.id,
                current_role_code=params.role_code,
                union_user_id=current_user_info.union_user_info.id,
            ),
        )
    ...
    def edit_user(self, params: EditWebUserModel, current_user_info: ValidateTokenResModel):
        ...
        exist_union_user = self.__union_user_repository.get_by_id(
            union_user_id=params.union_user_uuid
        )
        if exist_union_user:
