...
from basic_module.model.union_user_model import EditUnionUserInfoModel, UnionUserModel, \
    UnionUserAuthCodeLoginParamsModel, UnionUserInfoModel, UnionUserLoginResultModel, \
    RefreshUnionUserAccessTokenViewModel, DeleteUnionUserModel, UnionUserPageParamsModel
...
class UnionUserService:
        ...
        try:
            ...
            for user in payload.union_user_info.user_list:
                ...
                if role_data:
                    ...
                    if user_role_data:
                        user.union_user_user_role_list = user_role_data.role_list
                        if user_role_data.name:
                            user.union_user_user_name = user_role_data.name
                    else:
                        user.union_user_user_role_list = []
                else:
    ...
    def logout(
        ...
        access_token = self.__redis_client.get_value(
            f"access_token_{space}:{current_user_info.union_user_info.id}_{scene}"
        )
        if access_token:
            self.__redis_client.delete_key(
                key=f"access_token_{space}:{current_user_info.union_user_info.id}_{scene}"
            )

        refresh_token = self.__redis_client.get_value(
            f"refresh_token_{space}:{current_user_info.union_user_info.id}_{scene}"
        )
        if refresh_token:
            self.__redis_client.delete_key(
                key=f"refresh_token_{space}:{current_user_info.union_user_info.id}_{scene}"
            )
    ...
    def get_page(self, params: UnionUserPageParamsModel):
        return self.__union_user_repository.get_union_user_page(
            params=params
        )
